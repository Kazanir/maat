...
hhvm ~/oss/batch-run.php --i-am-not-benchmarking --no-proxygen < ~/maat/tools/d8_progress.json > ~/maat/results/results_c1_${CURRENT_DATE}.json

echo -e "\n********************************************"
echo -e "***** Running D8 batch with concurrency 5..."

cp ~/maat/tools/oss/PerfC5.php ~/oss/base/PerfSettings.php
hhvm ~/oss/batch-run.php --i-am-not-benchmarking --no-proxygen < ~/maat/tools/d8_progress.json > ~/maat/results/results_c5_${CURRENT_DATE}.json

echo -e "\n*********************************************"
echo -e "***** Running D8 batch with concurrency 20..."

cp ~/maat/tools/oss/PerfC20.php ~/oss/base/PerfSettings.php
hhvm ~/oss/batch-run.php --i-am-not-benchmarking --no-proxygen < ~/maat/tools/d8_progress.json > ~/maat/results/results_c20_${CURRENT_DATE}.json
